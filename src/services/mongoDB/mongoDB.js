...
async function removeUsers(user_id_list) {
    ...
    await userInfo.deleteMany({user_id: {$in: user_id_list}});
    await userGameInfo.deleteMany({user_id: {$in: user_id_list}});
}
...
async function removeGames(game_id_list) {
    ...
    await gameInfo.deleteMany({ game_id: {$in: game_id_list} });
    await userGameInfo.deleteMany({game_id: {$in: game_id_list}});
}
//adds a game (or a list of) to the user profile
async function addGamesToUser(user_id, game_id_list) {
    try {
        game_id_list = await mongoAssist.gameListSetup(game_id_list);
        const data = game_id_list.map(game_id => ({ user_id: user_id, game_id: game_id }))
...
async function removeGamesFromUser(user_id, game_id_list) {
    try {
        game_id_list = await mongoAssist.gameListSetup(game_id_list);
        await userGameInfo.deleteMany({user_id: user_id, game_id: {$in: game_id_list}});
    }
    catch (error) {
...
async function getProfile(user_id=null, username=null) {
    try {
        ...
        const userInfoDoc = await userInfo.findOne(query, {projection: {_id: 0}});
        user_id = userInfoDoc.user_id;

        
        const userGameLink = await userGameInfo.find({user_id: user_id},{ projection: {_id: 0}}).toArray();
        const tempIDLink = mongoAssist.toExtract(userGameLink, 'game_id');

        const userGames = await gameInfo.find({game_id: {$in: tempIDLink}}, { projection: { date: 0, _id: 0}}).toArray();
        const gameNames = mongoAssist.toExtract(userGames, 'game_name');
        const gameScores = mongoAssist.toExtract(userGames, 'game_score');

        gameScores.forEach(score => {totalScore += parseInt(score, 10);});
        userDoc = {...userInfoDoc, totalScore: totalScore, games: gameNames};
...
async function createNewUser(username, email_addr, games_id_list) {
    try {
        ...
        const existingUser = userInfo.findOne({ 'username': username });
        const existingEmail = userInfo.findOne({ 'email_addr': email_addr });
...
async function introduceGameToCatalog(game_name, game_score) {
    try {
        ...
        const existingGame = gameInfo.findOne({ 'game_name': game_name });
...
async function getGamesFromCatalog(game_names_list, return_game_names=false) {
    ...
    if (game_names_list != undefined) {
        try{
            ...
            allDocuments = await gameInfo.find({ game_name: { $in: game_names_list} }, { projection: { game_id: 1, game_name: 1, _id: 0 } }).toArray();
        ...
        try {
            console.log('\ngrabbing all games');
            allDocuments = await gameInfo.find({}, { projection: { game_id: 1, game_name: 1, _id: 0 }}).toArray();
            id_list = mongoAssist.getIDFromDB(allDocuments);
        } 
        catch (error) {
